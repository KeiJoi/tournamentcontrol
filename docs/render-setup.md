...
2. Set the production branch to `main`, the build command to `npm ci --include=dev && npm run build`, and the start command to `npm run start --workspace=@tournament-control/server`. The application still runs with `NODE_ENV=production`; the explicit install flag only makes TypeScript and Vite available during the build.
