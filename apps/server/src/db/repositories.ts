...
export class BracketRepository {
  ...
  listMatches(tournamentId: string): Match[] { return this.database.prepare(`SELECT ${matchColumns} FROM matches WHERE tournament_id = ? ORDER BY (SELECT round_number FROM rounds WHERE rounds.id = matches.round_id), position`).all(tournamentId) as Match[]; }
